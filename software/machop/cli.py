...
class Main:
    arguments = {
        ...
        ('-p', '--pretrained'): {
            'action': 'store_true',
            'help': 'Use pretrained model from HuggingFace.',
        },
        ('-t', '--task'): {
            'type': str,
        ...
        ('-modify', '--modify'): {
            'action': 'store_true',
            'help': 'Name of the saved model to restore, from modifier.'
        },
        ('-i', '--interactive'): {
            'action': 'store_true',
            'help': 'Name of the saved model to restore, from modifier.'
    ...
    def parse(self):
        p = argparse.ArgumentParser(
            description='Millimeter Wave Radar Dataset.')
    ...
    def run(self):
        try:
            ...
            logging.error(f'Unkown action {self.a.action!r}, '
    ...
    def setup_model_and_data(self, a):
        ...
        if a.model in nlp_models:
            model = model_inst_fn(name=a.model,
                                  task=a.task,
                                  info=info,
            ...
            if a.model in manual_models:
                model = model_inst_fn(info=info, config=a.config)
            else:
