...
def get_input_args_generator(model_name, task, data_module):
    data_module.prepare_data()
    data_module.setup()
    ...
    if wrapper_cls == VisionModelWrapper:
        for batch_x, _ in iter(data_module.train_dataloader()):
            input_args = [batch_x[[0], ...]]
            yield input_args
    elif wrapper_cls == NLPClassificationModelWrapper:
        for batch in iter(data_module.train_dataloader()):
            input_args = [
                batch["input_ids"][[0], ...],
                batch["attention_mask"][[0], ...],
            ]
