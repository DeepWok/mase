...
def emit_parameters_in_rom(node, param_name, file_name, data_name):
    """
...
def emit_node_parameters_in_rom(node, rtl_dir):
    """
    Enumerate parameters of the input node and emit a ROM block with handshake interface
    for each parameter
    """
    node_name = vf(node.name)
    for param_name, parameter in node.meta.module.named_parameters():
        if (
            ...
            emit_parameters_in_rom(node, param_name, verilog_name, data_name)
