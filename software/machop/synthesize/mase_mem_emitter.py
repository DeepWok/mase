...
def _get_ram_block_in_verilog(depth, awidth, dwidth, name, node_name):
    return f"""
`timescale 1 ns / 1 ps
module {node_name}_{name}_ram #(
    parameter DWIDTH   = {dwidth},
    parameter AWIDTH   = {awidth},
    parameter MEM_SIZE = {depth} 
) (
...
`timescale 1 ns / 1 ps
module {node_name}_{name}_source #(
    parameter DataWidth = 32'd{dwidth},
    parameter AddressRange = 32'd{depth},
    parameter AddressWidth = 32'd{awidth}
) (
    input reset,
    input clk,
    ...
    output logic [DataWidth - 1:0] q0,
    input logic [AddressWidth - 1:0] address1,
    input ce1,
    input we1,
    input logic [DataWidth - 1:0] d1,
    output logic [DataWidth - 1:0] q1
);

  seq_blocks_0_v1_V_ram seq_blocks_0_v1_V_ram_U (
      .clk(clk),
      .addr1(address1),
      .ce1(ce1),
      .we1(we1),
...
def _emit_hls_data_in_mem(node, file_name):
    """
    Emit data_in and data_out for the node
    """
    assert not os.path.isfile(
        file_name
    ), "ROM Verilog file already exists before emitting. Please check if there are files with the same name."
    ...
    # The depth of parameters matches with the input depth
    in_depth = math.prod(node.meta.parameters["common"]["args"]["data_in"]["size"])
    in_awidth = int(math.ceil(math.log2(in_depth))) + 1
    in_dwidth = node.meta.parameters["common"]["args"]["data_in"]["precision"][0]
    in_name = "data_in"
    out_depth = math.prod(node.meta.parameters["common"]["results"]["data_out"]["size"])
    out_awidth = int(math.ceil(math.log2(in_depth))) + 1
    out_dwidth = node.meta.parameters["common"]["results"]["data_out"]["precision"][0]
    out_name = "data_out"

    node_name = vf(node.name)
    node_param_name = f"{node_name}_data_in and {node_name}_data_out"
    time_to_emit = time.strftime("%d/%m/%Y %H:%M:%S")
...
// =====================================
"""
    ram_verilog += _get_ram_block_in_verilog(
        in_depth, in_awidth, in_dwidth, in_name, node_name
    )
    ram_verilog += _get_ram_block_in_verilog(
        out_depth, out_awidth, out_dwidth, out_name, node_name
    )
    with open(file_name, "w", encoding="utf-8") as outf:
        outf.write(ram_verilog)
    logger.debug(
        f"RAM modules data_in and data_out successfully written into {file_name}"
    )
    assert os.path.isfile(file_name), "ROM Verilog generation failed."
    os.system(f"verible-verilog-format --inplace {file_name}")
...
def emit_parameters_in_rom_hls(node, rtl_dir):
    ...
    for param_name, parameter in node.meta.module.named_parameters():
        if (
            ...
            assert False, "Emtting parameters in non-BRAM hardware is not supported."
    verilog_name = os.path.join(rtl_dir, f"{node_name}_data.sv")
    _emit_hls_data_in_mem(node, verilog_name)
