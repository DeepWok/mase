...
class MaseGraph:
    ...
    hw_parameters = {
        nn.Linear: [
            'ACT_WIDTH', 'W_WIDTH', 'OUTPUT_WIDTH', 'VECTOR_SIZE',
            'NUM_VECTORS', 'PARALLELISM', 'COMPUTE_TYPE'
        ],
    ...
    def _pre_synthesis_check(self):
        ...
        logging.debug(f'''Network overview: 
{count}
Layer types: 
{layer_types}''')
    ...
    def _add_hw_parameters(self):
        for node in self.fx_graph.nodes:
            ...
            if type(node_layer) not in self.hw_parameters:
                ...
                node.meta = dict.fromkeys(self.hw_parameters[type(node_layer)],
                                          None)
    ...
    def verify(self):
        for node in self.fx_graph.nodes:
            ...
            assert node.meta[
                'target'] in self.compile_targets, 'Unknown compilation target found: {}'.format(
                    node.meta['target'])
    ...
    def _verify_parameters_linear(self):
        ...
        for node in self.fx_graph.nodes:
            ...
            if isinstance(node_layer, target):
                ...
                assert ACT_WIDTH > 0, f'Invalid hardware parameter ACT_WIDTH = {ACT_WIDTH}. {node}'
                W_WIDTH = node.meta['W_WIDTH']
                assert W_WIDTH > 0, f'Invalid hardware parameter W_WIDTH = {W_WIDTH}. {node}'
                OUTPUT_WIDTH = node.meta['OUTPUT_WIDTH']
                assert OUTPUT_WIDTH > 0, f'Invalid hardware parameter OUTPUT_WIDTH = {OUTPUT_WIDTH}. {node}'
                VECTOR_SIZE = node.meta['VECTOR_SIZE']
                assert VECTOR_SIZE > 0, f'Invalid hardware parameter VECTOR_SIZE = {VECTOR_SIZE}. {node}'
                NUM_VECTORS = node.meta['NUM_VECTORS']
                assert NUM_VECTORS > 0, f'Invalid hardware parameter NUM_VECTORS = {NUM_VECTORS}. {node}'
                PARALLELISM = node.meta['PARALLELISM']
                assert PARALLELISM > 0, f'Invalid hardware parameter PARALLELISM = {PARALLELISM}. {node}'
