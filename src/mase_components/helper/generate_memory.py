...
def inttobit(data_width: int, num: float, signed: bool = True):
    intbits = (
        BitArray(int=num, length=data_width)
        if signed
        else BitArray(uint=num, length=data_width)
    )
    return intbits
...
class GenerateMxIntSVLut(GenerateSVLut):
    def quant_profile(self, bin_in):
        in_man_width, in_exp_width, out_man_width, out_exp_width = (
            self.parameter["in_man_width"],
            self.parameter["in_exp_width"],
            self.parameter["out_man_width"],
            self.parameter["out_exp_width"],
        )
    ...
    def generate_lut_address(self):
        ...
        upperbound_of_mx_output = (
            (2 ** (out_man_width - 1) - 1)
            / 2 ** (out_man_width - 1)
            * 2 ** (2 ** (out_exp_width - 1) - 1)
        )
        lowerbound_of_mx_output = (
            (1) / 2 ** (out_man_width - 1) * 2 ** (-(2 ** (out_exp_width - 1)))
        )
        ...
        _right = (
            (1 * 2 ** (out_man_width - 2) + 1) / 2 ** (out_man_width - 1) * 2 ** (1)
        )
