...
class CircularBufferTB(Testbench):
    def __init__(self, dut) -> None:
        ...
        self.assign_self_params(["DATA_WIDTH", "MAX_REPEAT", "MAX_SIZE"])
    ...
    def generate_inputs(self, num=10):
        ...
        for _ in range(num):
            inputs.extend(
                [random.randint(0, 2**self.DATA_WIDTH - 1) for _ in range(self.MAX_SIZE)]
            )
        return inputs

    def model(self, inputs):
        x = list(batched(inputs, n=self.MAX_SIZE))
        y = []
        for seq in x:
...
def generate_random_params():
    return {
        ...
        "MAX_REPEAT": random.randint(2, 6),
        "MAX_SIZE": random.randint(2, 6),
    }
...
def test_repeat_circular_buffer():
    mase_runner(
        module_param_list=[
            # Power of 2 params
            {"DATA_WIDTH": 8, "MAX_REPEAT": 2, "MAX_SIZE": 4},
            # Change data width
            {"DATA_WIDTH": 17, "MAX_REPEAT": 2, "MAX_SIZE": 4},
            # Non power of 2 repeats
            {"DATA_WIDTH": 32, "MAX_REPEAT": 3, "MAX_SIZE": 4},
            # Non power of 2 buffer size
            {"DATA_WIDTH": 32, "MAX_REPEAT": 2, "MAX_SIZE": 7},
