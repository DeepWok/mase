...
def mxint_softmax(x, q_config):
    ...
    for i in range(shape):
        ...
        fixed_r, integer_n = quantized_range_reduction(
            mx, ex, in_man_width, data_out_n_width
        )
    ...
    m_sum, e_sum = MxIntAccumulator(
        torch.stack(list_of_mexps), torch.stack(list_of_eexps)
    )
    ...
    for i in range(shape):
        _, mout[i], eout[i] = mxint_quantize(
            pre_cast_out[i], data_out_man_width, data_out_exp_width
        )
    return mout, eout
        ...
        for _ in range(self.num):
            ...
            for i in range(shape):
                _, mdata_in, edata_in = mxint_quantize(
                    data[i], q_config["in_man_width"], q_config["in_exp_width"]
                )
...
async def check_signal(dut):
    ...
    while True:
        ...
        if dut.acc_data_out_valid.value == 1 and dut.acc_data_out_ready.value == 1:
            print(
                "acc_data_out: mdata =",
                [x.signed_integer for x in dut.acc_mdata_out.value],
                "edata =",
                dut.acc_edata_out.value,
            )
        print("---")
