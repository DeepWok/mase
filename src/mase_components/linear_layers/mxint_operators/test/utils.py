...
class MXIntLinearHardware(_LinearBase):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = wrapped_mxint_linear_hardware(in_x, in_w, in_bias, self.in_features, self.out_features, self.config)
...
def wrapped_mxint_linear_hardware(x,w,bias,in_features, out_features, config):
    ...
    in_config = {
        "x_config": {
            ...
            "parallism_dim_0": config["data_in_parallelism_dim_0"],
            "parallism_dim_1": config["data_in_parallelism_dim_1"],
            "depth_dim_0": in_features//config["data_in_parallelism_dim_0"], 
            "depth_dim_1": n//config["data_in_parallelism_dim_1"], 
            "dim_0": in_features, 
            "dim_1": n, 
        },
        "w_config": {
            ...
            "parallism_dim_0": config["weight_parallelism_dim_0"],
            "parallism_dim_1": config["weight_parallelism_dim_1"],
            "depth_dim_0": in_features//config["weight_parallelism_dim_0"], 
            "depth_dim_1": out_features//config["weight_parallelism_dim_1"], 
            "dim_0": in_features, 
            "dim_1": out_features, 
        },
        "bias_config": {
            ...
            "parallism_dim_0": config["bias_parallelism_dim_0"],
            "parallism_dim_1": 1,
            "depth_dim_0": out_features//config["bias_parallelism_dim_0"], 
            "depth_dim_1": 1, 
            "dim_0": out_features, 
            "dim_1": 1, 
        },
        "out_config": {
            ...
            "parallism_dim_0": config["data_out_parallelism_dim_0"],
            "parallism_dim_1": config["data_out_parallelism_dim_1"],
            "depth_dim_0": out_features//config["data_out_parallelism_dim_0"], 
            "depth_dim_1": n//config["data_out_parallelism_dim_1"], 
            "dim_0": out_features, 
            "dim_1": n, 
        },
    }
    ...
    reshaped_mout = mout.reshape(out_config["depth_dim_1"], out_config["parallism_dim_1"], out_config["depth_dim_0"], out_config["parallism_dim_0"]).permute(0,2,1,3)
    reshaped_out = reshaped_mout * 2**(eout[:, :, None, None] - config["data_out_width"] + 1)
    out = reshaped_out.reshape(out_config["depth_dim_1"], out_config["depth_dim_0"], out_config["parallism_dim_1"], out_config["parallism_dim_0"]).permute(0,2,1,3)
...
def mxint_linear_hardware(x, w, bias, config):
    ...
    def block_wise_reshape_tensor(x, x_config):
        reshaped_x = x.reshape(x_config["depth_dim_1"],x_config["parallism_dim_1"], x_config["depth_dim_0"],x_config["parallism_dim_0"]).permute(0,2,1,3)
        ...
        return reshaped_x
    # assume 2 dimensional input
    assert x_config["depth_dim_0"] == w_config["depth_dim_0"], "need to check the setting of dim"
    assert x_config["parallism_dim_0"] == w_config["parallism_dim_0"], "need to check the setting of dim"
    ...
    man_out = torch.zeros(x_config["depth_dim_1"], w_config["depth_dim_1"], x_config["parallism_dim_1"] * w_config["parallism_dim_1"])
    ...
    for i in range(x_config["depth_dim_1"]):
        for j in range(w_config["depth_dim_1"]):
            partial_man_out = torch.zeros(w_config["depth_dim_0"], x_config["parallism_dim_1"], w_config["parallism_dim_1"])
            ...
            for k in range(x_config["depth_dim_0"]):
                ...
                partial_man_out[k], partial_exp_out[k] = DotProductCore(mx_block, ex_block, mw_block,ew_block)
            acc_man_out, acc_exp_out = MxIntAccumulator(partial_man_out.reshape(w_config["depth_dim_0"], -1), partial_exp_out)
            print("software: acc_man, exp", acc_man_out, acc_exp_out)
            if bias!=None:
                bias_config = config["bias_config"]
                mbias, ebias = bias
                reshaped_mbias = mbias.reshape(w_config["depth_dim_1"], w_config["parallism_dim_1"])
                reshaped_ebias = ebias.reshape(w_config["depth_dim_1"])
                shifted_value = reshaped_ebias[j] - acc_exp_out + x_config["width"] + w_config["width"]  - 2 - (bias_config["width"] -1)
                shifted_bias = reshaped_mbias[j].repeat(x_config["parallism_dim_1"]) * 2**(shifted_value)
                acc_man_out = shifted_bias + acc_man_out
            man_out[i][j], exp_out[i][j] = MxIntCast(acc_man_out, acc_exp_out, 
                            {
                                "in_width": x_config["width"] + w_config["width"] + ceil(log2(x_config["dim_0"])),
                                "in_frac_width": x_config["width"] + w_config["width"] - 2,
                                "in_exponent_width": max(x_config["exponent_width"], w_config["exponent_width"]) + 1,
                                "out_width": out_config["width"],
                                "out_exponent_width": out_config["exponent_width"],
                            })
    man_out = man_out.reshape(x_config["depth_dim_1"], w_config["depth_dim_1"], x_config["parallism_dim_1"], w_config["parallism_dim_1"]).permute(0, 2, 1, 3).reshape(x_config["dim_1"], w_config["dim_1"])
...
def MXIntMatmulHardware(man_x, exp_x, man_y, exp_y, x_config, y_config, out_config):
    ...
    man_out = torch.zeros(x_config["depth_dim_1"], y_config["depth_dim_0"], x_config["parallism_dim_1"] * y_config["parallism_dim_0"])
    ...
    for i in range(x_config["depth_dim_1"]):
        for j in range(y_config["depth_dim_0"]):
            ...
            man_out[i][j], exp_out[i][j] = MxIntCast(acc_man_out, acc_exp_out, 
                            {
                                "in_width": x_config["width"] + y_config["width"] + ceil(log2(x_config["dim_0"])),
                                "in_frac_width": x_config["width"] + y_config["width"] - 2,
                                "in_exponent_width": max(x_config["exponent_width"], y_config["exponent_width"]) + 1,
                                "out_width": out_config["width"],
                                "out_exponent_width": out_config["exponent_width"],
                            })
    man_out = man_out.reshape(x_config["depth_dim_1"], y_config["depth_dim_0"], x_config["parallism_dim_1"], x_config["parallism_dim_0"]).permute(0, 2, 1, 3).reshape(x_config["dim_1"], y_config["dim_0"])
    return man_out, exp_out
