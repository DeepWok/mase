...
from hls.bfp_arith import mxfp_linear2d_gen
from hls import HLSWriter


def mxfp_linear2d_dse(mode=None, top=None, threads=32):
                    ...
                    for w_row in w_rows:
                        for w_exp_width in w_exp_widths:
                            for w_man_width in w_man_widths:
                                ...
                                file_name = f"x{i}_mxfp_linear2d_{x_row}_{x_col}_{x_exp_width}_{x_man_width}_{w_row}_{w_col}_{w_exp_width}_{w_man_width}"
                                tcl_path = os.path.join(top, f"{file_name}.tcl")
                                file_path = os.path.join(top, f"{file_name}.cpp")
                                if mode in ["codegen", "all"]:
                                    writer = HLSWriter()
                                    writer = mxfp_linear2d_gen(
                                        writer,
                                    ...
                                    top_name = f"mxfp_linear2d_{writer.op_id-1}"
    ...
    if mode in ["codegen", "all"]:
        ...
        bash_gen(commands, top, "mxfp_linear2d")

    if mode in ["report", "all"]:
        # Export regression model data points to csv
        csv_gen(data_points, top, "mxfp_linear2d_hw")

    if mode in ["count_loc", "all"]:
        # Export regression model data points to csv
        csv_gen(loc_points, top, "mxfp_linear2d_loc")
