...
class LlamaMLAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.absorb:
            query_states = nn.functional.dropout(
                query_states, p=self.kv_dropout, training=self.training
            )
            query_states = query_states.view(
                bsz, q_len, self.num_heads, self.num_key_value_heads * self.head_dim
            ).transpose(1, 2)
            key_states = key_states.unsqueeze(
                1
            )  # (bsz, 1, kv_len, self.num_key_value_heads*self.head_dim)
            value_states = nn.functional.dropout(
                value_states, p=self.kv_dropout, training=self.training
            ).unsqueeze(
                1
            )  # (bsz, 1, kv_len, self.num_key_value_heads*self.head_dim)
        else:
            query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim)
            k_up_weight = self.k_up_proj.weight.view(
            ...
            key_states = nn.functional.dropout(
                key_states, p=self.kv_dropout, training=self.training
            ).unsqueeze(1)
            value_states = nn.functional.dropout(
                value_states, p=self.kv_dropout, training=self.training
            )
        ...
        if self.rope_repeat:
            query_states, key_states = repeat_apply_rotary_pos_emb(
...
class LlamaSdpaMLAttention(LlamaMLAttention):
    ...
    # Adapted from LlamaMLAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
