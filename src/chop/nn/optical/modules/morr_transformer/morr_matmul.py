...
__all__ = ["AllPassMORRCirculantMatMuls"]


class AllPassMORRCirculantMatMuls(ONNBaseLayer):
    """
    ...
    def __init__(
        ...
        super(AllPassMORRCirculantMatMuls, self).__init__()
    ...
    def load_compressed_weight(self, weight: Tensor) -> None:
        """
        load weight data from torch.linear module.weight.data
        """
        assert weight.shape == (self.out_features, self.in_features), (
            f"Expected {(self.out_features, self.in_features)}, got {weight.shape}"
        )

        W_padded = weight.new_zeros((self.out_features_pad, self.in_features_pad))
        W_padded[: weight.size(0), : weight.size(1)] = weight

        new_weight = weight.new_zeros((self.grid_dim_y, self.grid_dim_x, self.miniblock))
        for p in range(self.grid_dim_y):
            for q in range(self.grid_dim_x):
                for k in range(self.miniblock):
                    row_idx = p * self.miniblock + k # The row in W_padded:
                    col_start = q * self.miniblock # The columns in W_padded:
                    col_end = (q + 1) * self.miniblock
                    block = W_padded[row_idx, col_start:col_end]

                    new_weight[p, q, k] = block.mean()
        bound = 1 / math.sqrt(self.miniblock)
        new_weight = torch.rand(
            (self.grid_dim_y, self.grid_dim_x, self.miniblock), 
            device=weight.device, 
            dtype=weight.dtype
        ) * 2 * bound - bound
        
        self.load_parameters({"weight": new_weight})
        
    def forward(self, X: Tensor, Y: Tensor) -> Tensor:
        """
        this module currently support 4-D multi-head attn MatMul only
        - x: input, [B, H, N, D]
        - y: weight, [B, H, D, N]
        """
        assert len(X.shape) == 4, f"Expected a 4-D tensor, got shape {X.shape}"
        B, H, N, D = X.shape
        out_rows = []
        
        for b in range(B):
            for h in range(H):
                self.load_compressed_weight(Y[b, h].t())
                x = X[b, h]
        ...
        out = torch.stack(out_rows, dim=0) # (B·H, N, N)
        out = out.view(B, H, N, self.out_features)
        return out