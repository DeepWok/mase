...
class AllPassMORRLinear(ONNBaseLayer):
    ...
    def _compute_single_pass(self, weight: Tensor, x: Tensor, morr_output_scale: Tensor) -> Tensor:
    ...
    def propagate_morr(
        ...
        result_pp = self._compute_single_pass(pos_weight, pos_x, morr_output_scale)# 1. W+X+
        result_np = self._compute_single_pass(neg_weight, pos_x, morr_output_scale)# 2. |W-|X+
        result_pn = self._compute_single_pass(pos_weight, neg_x, morr_output_scale)# 3. W+|X-|
        result_nn = self._compute_single_pass(neg_weight, neg_x, morr_output_scale)# 4. W-X-
