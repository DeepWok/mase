...
This is a 1:1 port of ``Plena-Acc-Sim/quantize/quantizer/mxfp/fake.py`` so
that MXFP outputs match the accelerator simulator byte-for-byte. The main
divergence from MASE's earlier OCP-MX-style impl:

  * scales = ``log2().ceil()``  (Plena, NOT ``floor()`` like the OCP MX
    standard) — Plena's shared exponent rounds the per-block max UP in
    log space, then divides the block by ``2**scale``.
  * No ``- el_exp_max_biased`` correction (Plena's scale is the raw log2
    of the per-block max, not the OCP "place-max-at-element-format-max"
    convention).
  * No subnormal / flush-to-zero handling (Plena delegates element
    quantization to ``minifloat_denorm_quantizer`` which handles its own
    underflow).
  * Returns float scales (log2 domain) and float minifloat-quantized
    elements — NOT uint8 encodings. Recompose via ``elements * 2**scales``.

If you need to change this back to OCP MX, re-introduce ``floor`` and the
biased-uint8 encoding — see the commented-out reference impl at the bottom
of Plena's fake.py for the OCP form.
"""
