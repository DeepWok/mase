"""Llama decoder layer with rounded vector residual boundaries."""
...
class LlamaDecoderLayerMinifloat(LlamaDecoderLayer):
    """Apply one FP_SETTING policy to both residual additions."""

    def __init__(self, config, layer_idx: int, q_config: dict = None):
    ...
    @classmethod
    def from_self(cls, layer: LlamaDecoderLayer, q_config: dict = None):
        new_layer = cls(
        ...
        new_layer = new_layer.to(device=parameter.device, dtype=parameter.dtype)
