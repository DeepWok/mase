...
def linearGradInteger(ctx, grad_output, config: dict = None):
    ...
    output_grad_width, output_grad_frac_width = config["output_grad_width"], config["output_grad_frac_width"]
    input_width, input_frac_width = config["data_in_width"], config["data_in_frac_width"]
    weight_width, weight_frac_width = config["weight_width"], config["weight_frac_width"]

    floor = config.get("floor", False)
    base_quantizer = integer_floor_quantizer if floor else integer_quantizer

    w_quantizer = partial(base_quantizer, width=weight_width, frac_width=weight_frac_width)
    x_quantizer = partial(base_quantizer, width=input_width, frac_width=input_frac_width)
    out_grad_quantizer = partial(base_quantizer, width=output_grad_width, frac_width=output_grad_frac_width)
