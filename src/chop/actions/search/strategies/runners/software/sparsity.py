...
from chop.passes.graph.analysis.add_metadata.add_software_metadata import (
...
class RunnerPrunedSparsity(SWRunnerBase):
    ...
    def __call__(
        self, data_module, model: MaseGraph, sampled_config
    ) -> dict[str, float]:
        ...
        for node in model.fx_graph.nodes:
            if get_mase_op(node) in ["linear", "conv2d", "conv1d"]:
                meta = node.meta["mase"]
                num_weights = meta.parameters["common"]["args"][
                    "parametrizations.weight.original"
                ]["value"].numel()
                num_activations = meta.parameters["common"]["args"]["data_in_0"][
                    "value"
                ].numel()
                ...
                sparsity_data[node.target] = {
                    "weight_sparsity": meta.parameters["software"]["args"]["weight"][
                        "sparsity"
                    ],
                    "activation_sparsity": meta.parameters["software"]["args"][
                        "data_in_0"
                    ]["sparsity"],
                }
        ...
        for node in element_data:
            ...
            pruned_weights += (
                element_data[node]["weights"] * sparsity_data[node]["weight_sparsity"]
            )
            pruned_activations += (
                element_data[node]["activations"]
                * sparsity_data[node]["activation_sparsity"]
            )
