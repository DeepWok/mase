    ...
    if module in module_map:
        for parameter, sharding_config in module_map[module].items():
            ...
            rlog(
                logger,
                rank,
                f"Distributing parameter {parameter} of module {module} to {placement}",
                level="debug",
            )
            try:
                ...
                rlog(
                    logger,
                    rank,
                    f"Error distributing parameter {parameter} of module {module}: {e}",
                    level="error",
                )
...
def device_fn(rank, world_size, model=None, device_mesh=None, module_map={}, inputs=[]):
    ...
    model = distribute_module(
        model,
        mesh,
        partial(dist_model_fn, rank=rank, module_map=module_map),
        input_fn=None,
        output_fn=None,
    )
    end = time()
