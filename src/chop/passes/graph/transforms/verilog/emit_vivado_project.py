...
def generate_tcl_script(top_name, vivado_project_path, include_groups, project_dir):
    logger.info(f"Writing Vivado project generation script: {vivado_project_path}/build.tcl")
...
def emit_vivado_project_transform_pass(graph, pass_args={}):
    ...
    vivado_project_path = os.path.join(project_dir, "hardware", f"{top_name}_build_project")
    os.makedirs(vivado_project_path, exist_ok=True)
    
    # * List include files
    include_groups = [
        f"{COMPONENTS_PATH / group / 'rtl'}"
        for group in mase_components.get_modules() if group != "vivado"
