...
if not check_deps_tensorRT_pass(silent=True):

    def FakeQuantizer(config: Dict):
        raise NotImplementedError(
            "TensorRT pass is unavailable because the required dependencies are not installed."
        )

    def Int8Calibrator(nCalibration, input_generator, cache_file_path):
        raise NotImplementedError(
            "TensorRT pass is unavailable because the required dependencies are not installed."
        )

    def PowerMonitor(config: Dict):
        raise NotImplementedError(
            "TensorRT pass is unavailable because the required dependencies are not installed."
        )
