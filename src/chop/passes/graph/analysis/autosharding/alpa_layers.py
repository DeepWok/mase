...
def get_valid_2d_shardings(node_meta, mesh, module):
    ...
    for perm in itertools.product(VALID_2D_TENSOR_SHARDINGS, repeat=2):
        ...
        if not is_fully_replicated(perm) and is_valid_sharding_pair(perm) and is_valid_2d_sharding(output_sharding):
            ...
            communication_cost_vector.append(get_communication_cost(perm, node_meta["mase"], mesh))
...
def get_valid_embedding_shardings(node_meta, mesh, module):
    ...
    valid_input_shardings = [{"data_in_0": (SpmdShard.R,) * data_in_rank, "weight": (SpmdShard.R,) * weight_rank}]
