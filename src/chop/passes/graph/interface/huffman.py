...
def find_module_of_parameter(model, full_param_name):
    ...
    submodule_path = parts[
        :-1
    ]  # Everything except the last part, which is the parameter name
...
def flatten_parameters(model, mg):
    ...
    get_named_params = lambda name, params: [
        param for param in params if name in param[0]
    ]
    ...
    for node in mg.fx_graph.nodes:
        ...
        if node.target in mg.modules:
            ...
            for name, _ in module.named_parameters():
                ...
                if (
                    isinstance(
                        actual_module, torch.nn.utils.parametrize.ParametrizationList
                    )
                    and "weight" in actual_name
                ):
                    actual_param = actual_param * actual_module[0].mask.to(
                        actual_param.dtype
                    )
    ...
    for node in mg.fx_graph.nodes:
        ...
        if node.target in mg.modules:
            ...
            for name, _ in module.named_parameters():
                ...
                encoded_values = "".join(
                    [global_huffman_codes[val] for val in actual_param_values]
                )
                ba = bitarray.bitarray(encoded_values)

                # Save encoded parameters
                encoded_file_path = os.path.join(
                    save_dir, "parameters", f"{node.target}.{name}.{len(ba)}.bin"
                )
