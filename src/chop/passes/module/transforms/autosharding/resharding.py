...
class ReshardingWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        required_placement = placement_from_sharding_config(
            self.resharding_config["data_in_0"]
        )
        if x.placements != required_placement:
            rlog(
                logger,
                rank,
                f"For module {self.module}, resharding tensor x from {x.placements} to {required_placement}",
                level="debug",
            )
    ...
    for node in mg.fx_graph.nodes:
        ...
        if module is not None:
            ...
            deepsetattr(
                mg.model,
                node.target,
                ReshardingWrapper(device_mesh, module, resharding_config),
            )
