# Chop: the software stack for MASE

![Chop](../docs/imgs/machop.png)

Chop is the Python stack behind MASE: CLI (`ch`), training and testing actions, graph passes, datasets, and model tooling. For user-facing documentation, see the [MASE docs](https://deepwok.github.io/mase/).
...
Chop exposes four actions: train, test, transform and search. Parameters for each action can be defined through command-line arguments or within a `toml` configuration file.
...
All arguments can be defined in a `toml` file and loaded using `--config`. See the [`configs`](https://github.com/DeepWok/mase/tree/main/configs) directory for example configuration files.
```bash
...
Chop supports PyTorch state dict checkpoint (`.ckpt`) and PyTorch Lightning checkpoint (`.ckpt`). The modified model (generated by transform action) is always saved and loaded in pickle format (`.pkl`).

#### Load checkpoint

Chop uses the action name (`train`/`test`/`transform`/`search`)*, `--load LOAD_NAME`, and `--load-type LOAD_TYPE` to infer when and how to load checkpoint. A proper `--load-type` is required if `--load LOAD_NAME` is specified.
...
- `pkl`: Chop software checkpoint containing a modified model.

The figure below outlines how Chop determines when and where to load.
...
- If `--load-type=hf` is used,
  ...
  - For patched models provided by MASE, `--pretrained` will loads the pretrained original model. For example `ch <ACTION> facebook/opt-125m@patched <DATASET> --pretrained --load-type=hf` will load Facebook's `opt-125m` checkpoint into the `facebook/opt-125m@patched` model.
...
- For Python: see [Python coding style](https://github.com/DeepWok/mase/blob/main/docs/source/modules/documentation/specifications/Python-coding-style-specifications.md).
...
- Tune a pre-trained `opt` model on `wikitext2` on GPU
  ...
  ```bash
  vim src/chop/dataset/nlp/language_modeling.py
  ```
...
See the [open issues](https://github.com/DeepWok/mase/issues) for a full list of proposed features (and known issues).
...
The following model categories are supported. For the full registry in code, browse [`src/chop/models`](https://github.com/DeepWok/mase/tree/main/src/chop/models) on GitHub.
...
The following dataset categories are supported. For the full registry in code, browse [`src/chop/dataset`](https://github.com/DeepWok/mase/tree/main/src/chop/dataset) on GitHub.
