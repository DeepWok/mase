...
def split_matrix(
    ...
    for i in range(depth_dim1):
        for j in range(depth_dim0):
            block = x[i*compute_dim1:(i+1)*compute_dim1,
...
def rebuild_matrix(
    ...
    for i in range(depth_dim1):
        for j in range(depth_dim0):
            ...
            arr[i*compute_dim1:(i+1)*compute_dim1,
                j*compute_dim0:(j+1)*compute_dim0] = block
            block_ind += 1
    return arr.int()
...
def gen_random_matrix_input(
    ...
    return split_matrix(x, total_dim0, total_dim1,
                        compute_dim0, compute_dim1)

def matrix_mult_model(
    a_total_dim0, a_total_dim1, a_compute_dim0, a_compute_dim1,
    b_total_dim0, b_total_dim1, b_compute_dim0, b_compute_dim1,
    c_total_dim0, c_total_dim1, c_compute_dim0, c_compute_dim1,
    a_width, a_frac_width,
    b_width, b_frac_width,
    out_width, out_frac_width, out_symmetric,
    a_input, b_input
):
    A = rebuild_matrix(a_input, a_total_dim0, a_total_dim1,
                        a_compute_dim0, a_compute_dim1)
    B = rebuild_matrix(b_input, b_total_dim0, b_total_dim1,
                        b_compute_dim0, b_compute_dim1)
    ...
    return split_matrix(C_unsigned_rep, c_total_dim0, c_total_dim1,
                        c_compute_dim0, c_compute_dim1)


# def generate_driver_input_4d(
#     total_shape,
#     compute_shape,
#     width,
#     frac_width
# ):
#     assert len(total_shape) == 4, "Total shape must be 4d shape!"
#     assert len(compute_shape) == 4, "Compute shape must be 2d window!"
#     batches = total_shape[0]
#     channels = total_shape[1]
#     inputs = list()
#     for _ in range(batches * channels):
#         inputs.extend(gen_random_matrix_input(
#             total_dim0=total_shape[3],
#             total_dim1=total_shape[2],
#             compute_dim0=compute_shape[1],
#             compute_dim1=compute_shape[0],
#             width=width,
...
# def input_reconstruction_4d(
#     inputs,
#     total_shape,
#     compute_shape,
# ):
#     depth_dim0 = total_shape[3] // compute_shape
