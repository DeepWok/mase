    ...
    def __init__(self, dropout_rate, filters, num_classes):
        ...
        self.layers = nn.ModuleList(
            [
                ...
                nn.LSTM(
                    input_size=filters, hidden_size=500, batch_first=True, dropout=0.0
                ),
...
ops = {
    "linear": Linear,
    "conv5": functools.partial(
        PadConvRelu, kernel_size=5, dilation=1, strides=1, groups=100
    ),
    "conv5d2": functools.partial(
        PadConvRelu, kernel_size=5, dilation=2, strides=1, groups=100
    ),
    "conv7": functools.partial(
        PadConvRelu, kernel_size=7, dilation=1, strides=1, groups=100
    ),
    "conv7d2": functools.partial(
        PadConvRelu, kernel_size=7, dilation=2, strides=1, groups=100
    ),
