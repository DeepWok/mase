...
def graph_hash(g):
    ...
    def hash_module(matrix, labelling):
        ...
        assert (
            len(in_edges) == len(out_edges) == len(labelling)
        ), f"{labelling} {matrix}"
...
class Dataset(_Dataset):
    ...
    def __init__(
        self, dataset_files, devices_files=None, static_info=None, validate_data=True
    ):
        """Create a new dataset by loading data from the provided list of files.
        ...
        if devices_files:
            self.bench_info = BenchmarkingDataset(
                devices_files, validate_data=validate_data
            )
        if static_info:
    ...
    def full_info(
        self, arch, seed=None, devices=None, include_static_info=None, return_dict=True
    ):
    ...
    def full_info_by_graph(
        self, graph, seed=None, devices=None, include_static_info=None, return_dict=True
    ):
    ...
    def test_acc(self, arch, seed=None):
        ...
        """
        info = self.full_info(
            arch, seed=seed, devices=False, include_static_info=False, return_dict=False
        )
        if info is None:
            return None
        return info[2]
