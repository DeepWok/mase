...
class Resize(T.Resize):
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            + f"(input_size={self.input_size}, target_size={self.target_size}, interpolation={interpolate_str}"
        )
...
def load_class_object_logits(label_path, selected=False, normalize=True, final5k=False):
    ...
    if selected:
        selection_file = (
            os.path.join(lib_data_dir, "data", "class_object_final5k.npy")
            if final5k
            else os.path.join(lib_data_dir, "data", "class_object_selected.npy")
        )
...
def load_class_object_label(label_path, selected=False, final5k=False):
    logits = load_class_object_logits(
        label_path, selected=selected, normalize=True, final5k=final5k
    )
...
def load_class_scene_logits(label_path, selected=False, normalize=True, final5k=False):
    ...
    if selected:
        selection_file = (
            os.path.join(lib_data_dir, "data", "class_scene_final5k.npy")
            if final5k
            else os.path.join(lib_data_dir, "data", "class_scene_selected.npy")
        )
        selection = np.load(selection_file)
        logits = logits[selection.astype(bool)]
        if normalize:
...
class MakeJigsawPuzzle(object):
    ...
    def __call__(self, sample, task_name):
        ...
        if task_name == "jigsaw":
            ...
            if self.totensor:
                raws = torch.empty(
                    9, 3, self.tile_dim[0], self.tile_dim[1], dtype=torch.float32
                )
                images = torch.empty(
                    9, 3, self.tile_dim[0], self.tile_dim[1], dtype=torch.float32
                )
...
def get_room_layout_cam_mat_and_ranges(view_dict, make_x_major=False):
    ...
    def make_world_x_major(view_dictx):
        ...
        for world_rotx in apply_90_deg_rot_k_times:
            global_x_in_cam = rotate_world_to_cam(world_rotx.dot(global_x), view_dictx)
            # Project onto camera's horizontal (xz) plane
            degrees_away = math.degrees(
                math.acos(np.dot(global_x_in_cam, -axes_xyz[2]))
            )
            best = min(
                best, (degrees_away, np.linalg.inv(world_rotx))
            )  # python is neat
...
def point_info2room_layout(label_path):
    ...
    camera_matrix_euler = transforms3d.euler.mat2euler(
        camera_matrix[:3, :3], axes="sxyz"
    )
    ...
    bbox_cam = np.hstack((cube_center, camera_matrix_euler, x_scale, y_scale, z_scale))
