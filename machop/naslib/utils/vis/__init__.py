    ...
    for graph in optimizer.graph._get_child_graphs(single_instances=True):
        for u, v, edge_data in graph.edges.data():
            if edge_data.has("alpha"):
                ...
                disc_weights = torch.mean(
                    edge_weights.detach().reshape(-1, steps_per_epoch, num_alphas),
                    axis=1,
                ).cpu()
                soft_weights = torch.softmax(disc_weights, dim=-1).numpy()

                cell_name = (
                    edge_data["cell_name"] if hasattr(edge_data, "cell_name") else ""
                )
                edge_info[(u, v, cell_name)] = {}
                edge_info[(u, v, cell_name)]["op_names"] = [
                    op.get_op_name for op in edge_data.op.get_embedded_ops()
                ]
...
def plot_architectural_weights(config, optimizer, max_rows=4):
    ...
    for start_id in range(0, len(edge_info.keys()), max_rows):
        ...
        logger.info(
            f"Creating plot {config.save}/arch_weights_{start_id+1}to{start_id+n_rows}.png"
        )
        ...
        for ax_id, (u, v, cell_name) in enumerate(
            list(edge_info.keys())[start_id : start_id + n_rows]
        ):
            map = sns.heatmap(
            ...
            axes[ax_id].set_yticklabels(
                op_names, rotation=360, fontdict=dict(fontsize=fontsize)
            )
            if cell_name != "":
        ...
        fig.suptitle(
            f"optimizer: {config.optimizer}, search space: {config.search_space}, dataset: {config.dataset}, seed: {config.seed}"
        )
        fig.tight_layout()
        fig.savefig(
            f"{config.save}/arch_weights_{start_id+1}to{start_id+n_rows}.png", dpi=300
        )
