...
class ProxSGD(Optimizer):
    ...
    def __init__(
        ...
        defaults = dict(
            lr=lr, momentum=momentum, clip_bounds=clip_bounds, weight_decay=weight_decay
        )
            ...
            for x in group["params"]:
                ...
                if group["weight_decay"] is not None:  # operations are prunable
                    if self.normalization == "none":  # no normalization
                        ...
                        self.normalization == "mul"
                    ):  # normalize the weight decay by multiplying operation_dimension**exponent
                        a = (
                            group["weight_decay"]
                            * torch.pow(dim_group, self.normalization_exponent)
                            / tau
                        )
                    elif (
                        self.normalization == "div"
