...
class StatisticsEvaluator(object):
    ...
    def adapt_search_space(
        ...
        max_size = max(
            self.max_set_size, self.max_nbhd_trials, self.max_autocorr_trials
        )
    ...
    def evaluate_autocorr(self):
        ...
        logger.info(
            "autocorr mean: {}, std: {}".format(
                self.results["autocorr_mean"][-5:], self.results["autocorr_std"][-5:]
            )
        )
