...
if (
    config.search.acq_fn_optimization == "random_sampling"
    and config.search.use_zc_api == True
):
...
if (
    ...
    logging.warning(
        "Using ZC API with a acquisition optimization strategy that is not Random Sampling"
    )
