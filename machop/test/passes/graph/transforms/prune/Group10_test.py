...
# Load test config
config = toml.load(
    "/home/lycho-_-/文档/mase/machop/configs/tests/prune/Group10/vgg7_retrain.toml"
)
...
def prepare_model(model_name, dataset_name, load_name):
    ...
    model = pre_transform_load(
        load_name=load_name, load_type="pl", model=model.to("cuda")
    )
    return model
...
def load_pruned_weights(model, layers, index):
    for _, layer in model.named_children():
        if isinstance(
            layer, (torch.nn.modules.conv.Conv2d, torch.nn.modules.linear.Linear)
        ):
...
def eval_pruning_retraining(
    model_name, dataset_name, original_path, pruned_state_dict, retrained_path
):
    accuracy_metrics = []
    num_para_metrics = []
    ...
    accruacy_after_retraining = calculate_accuracy(
        retrained_model.to("cuda"), testloader
    )
...
accuracy_metrics, num_para_metrics = eval_pruning_retraining(
    model_name, dataset_name, original_path, pruned_state_dict, retrained_path
)

print(
    "The accuracy before pruning: %.5f %%, accuracy after pruning: %.5f %%, accuracy after retraining: %.5f %%"
    % (*accuracy_metrics,)
)
print(
    "The number of parameters before pruning: %d, number of parameters after pruning: %d"
    % (*num_para_metrics,)
)
print(
    "Compression ratio of pruning is: %.2f %%"
    % ((num_para_metrics[0] / num_para_metrics[1]) * 100)
)
print(
    "Accuracy rentention rate before retraining: %.2f %%"
    % ((accuracy_metrics[1] / accuracy_metrics[0]) * 100)
)
print(
    "Accuracy rentention rate after retraining: %.2f %%"
    % ((accuracy_metrics[2] / accuracy_metrics[0]) * 100)
)
