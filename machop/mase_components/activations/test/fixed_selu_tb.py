...
async def test_fixed_tanh(dut):
    ...
    for i in range(87):
        ...
        scaled_value = np.round(tanh_value_numpy * (2 ** DATA_OUT_0_PRECISION_1)).astype(int)
        ...
        assert (
            (dut.data_out_0[0].value - scaled_value) <= (max_error* (2** (DATA_OUT_0_PRECISION_1)))
