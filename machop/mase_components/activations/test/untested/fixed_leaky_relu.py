...
class HardswishTB(Testbench):
    ...
    def exp(self, inputs):
        ...
        qclamp = qints.clamp(min=-2**(self.w-self.fracw-1), max = 2**(self.w-self.fracw-1)-1)
        

    def generate_inputs(self,w,fracw):
        self.dquantizer = partial(
            integer_quantizer, width=w, frac_width=fracw
        )
        self.w = w
        self.fracw = fracw
        ...
        intinp.clamp(min=-2**(self.w-fracw-1), max = 2**(self.w-fracw-1)-1)
        print(intinp)
        pdb.set_trace()
        return intinp
