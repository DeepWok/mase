...
#- Perhaps, instead of setting the agent action to the precision directly (eg. 2, 4, 8, 16bits) we can set the action to the precision change: 
# (eg. 0: decrease precision, 1: same, 2: increase precision)
...
class MixedPrecisionEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Compute the model's metrics for the current configuration
        ...
        cost = software_metrics['loss'] + 0.2 * hardware_metrics['average_bitwidth']
