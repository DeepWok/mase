...
OPS = {
    ...
    "avg_pool_3x3": lambda C_in, C_out, stride, affine: POOLING(
        C_in, C_out, stride, "avg"
    ),
    "max_pool_3x3": lambda C_in, C_out, stride, affine: POOLING(
        C_in, C_out, stride, "max"
    ),
    "nor_conv_7x7": lambda C_in, C_out, stride, affine: ReLUConvBN(
        C_in, C_out, (7, 7), (stride, stride), (3, 3), affine
    ),
    "nor_conv_3x3": lambda C_in, C_out, stride, affine: ReLUConvBN(
        C_in, C_out, (3, 3), (stride, stride), (1, 1), affine
    ),
    "nor_conv_1x1": lambda C_in, C_out, stride, affine: ReLUConvBN(
        C_in, C_out, (1, 1), (stride, stride), (0, 0), affine
    ),
    ...
    "conv_7x1_1x7": lambda C_in, C_out, stride, affine: Conv717(
        C_in, C_out, stride, affine
    ),
    "conv_3x1_1x3": lambda C_in, C_out, stride, affine: Conv313(
        C_in, C_out, stride, affine
    ),
}
...
class Conv313(nn.Module):
    def __init__(self, C_in, C_out, stride, affine):
        ...
        self.op = nn.Sequential(
            ...
            nn.Conv2d(
                C_in, C_out, (1, 3), stride=(1, stride), padding=(0, 1), bias=False
            ),
            nn.Conv2d(
                C_out, C_out, (3, 1), stride=(stride, 1), padding=(1, 0), bias=False
            ),
...
class Conv717(nn.Module):
    def __init__(self, C_in, C_out, stride, affine):
        ...
        self.op = nn.Sequential(
            nn.ReLU(inplace=False),
            nn.Conv2d(
                C_in, C_out, (1, 7), stride=(1, stride), padding=(0, 3), bias=False
            ),
            nn.Conv2d(
                C_out, C_out, (7, 1), stride=(stride, 1), padding=(3, 0), bias=False
            ),
            nn.BatchNorm2d(C_out, affine=affine),
        )
