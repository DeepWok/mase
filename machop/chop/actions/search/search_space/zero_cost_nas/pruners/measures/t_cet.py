...
@measure(
    "tcet_syn_none", bn=True, mode="none", mt="synflow"
)  # change bn from False to True, in order to avoid lnwot changes. We will manually remove bn in compute_tcet_score set synflow WITHOUT bn.
@measure(
    "tcet_syn_log", bn=True, mode="log", mt="synflow"
)  # change bn from False to True, in order to avoid lnwot changes. We will manually remove bn in compute_tcet_score set synflow WITHOUT bn.
@measure(
    "tcet_syn_log1p", bn=True, mode="log1p", mt="synflow"
)  # change bn from False to True, in order to avoid lnwot changes. We will manually remove bn in compute_tcet_score set synflow WITHOUT bn.
@measure(
    "tcet_syn_norm", bn=True, mode="norm", mt="synflow"
)  # change bn from False to True, in order to avoid lnwot changes. We will manually remove bn in compute_tcet_score set synflow WITHOUT bn.
...
@measure(
    "t_cet", bn=True, mode="none", mt="synflow"
)  # change bn from False to True, in order to avoid lnwot changes. We will manually remove bn in compute_tcet_score set synflow WITHOUT bn.
def compute_tcet_score(
    ...
    lnwot_scores = compute_naswot(
        net1, inputs, targets, loss_fn, split_data, layerwise=True, return_Kmats=False
    )
    del net1
    ...
    net2 = net.get_prunable_copy(
        bn=True if mt == "snip" else False
    )  # manually keep bn in lnwot, and remove bn in synflow
    mt_scores = compute_snr_score(
        net2, inputs, targets, loss_fn, split_data, mode=mode, mt=mt
    )
    del net2
    torch.cuda.empty_cache()
    gc.collect()
...
def compute_snr_score(
    ...
    if mt == "synflow":
        ...
        snip_scores = compute_snip_per_weight(
            net, inputs, targets, mode="param", split_data=split_data, loss_fn=loss_fn
        )
    ...
    for mt_score in mt_scores:
        ...
        sigma = torch.std(s)
        sigma = torch.tensor(0.0) if torch.isnan(sigma) else sigma
