...
def channel_l1_weight_multi_layer(
    tensor: torch.Tensor, next_tensor: torch.Tensor, info: dict, sparsity: float
) -> torch.Tensor:
    ...
    if next_tensor != None:
        ...
        final_flattened_l1_norms = (
            0.5 * flattened_l1_norms + 0.5 * flattened_next_l1_norms
        )
        try:
...
def channel_l2_weight_multi_layer(
    tensor: torch.Tensor, next_tensor: torch.Tensor, info: dict, sparsity: float
) -> torch.Tensor:
    # multi-layer channel pruning
    ...
    if next_tensor != None:
        ...
        final_flattened_l2_norms = (
            0.5 * flattened_l2_norms + 0.5 * flattened_next_l2_norms
        )
        try:
...
def channel_similarity_feature_map(
    tensor: torch.Tensor, info: dict, sparsity: float
) -> torch.Tensor:
    ...
    cosine_similarity_matrix = torch.mm(
    ...
    triu_indices = torch.triu_indices(
        cosine_similarity_matrix.size(0), cosine_similarity_matrix.size(1), offset=1
    )
...
def global_activation_l1(tensor: torch.Tensor, info: dict, sparsity: float):
    ...
    flattened_tensors = [
        tensor.abs().sum(dim=(1, 2, 3)).flatten() for tensor in tensors
    ]
    flattened_tensors = torch.cat(flattened_tensors)
    try:
        threshold = torch.quantile(flattened_tensors, sparsity)
...
weight_criteria_map = {
    "local": {
        "elementwise": {"random": random, "l1-norm": l1_weight, "l2-norm": l2_weight},
        "kernelwise": {"l1-norm": kernel_l1_weight, "l2-norm": kernel_l2_weight},
        "channelwise": {
            "l1-norm-multi": channel_l1_weight_multi_layer,
            "l2-norm-multi": channel_l2_weight_multi_layer,
            "l1-norm-single": channel_l1_weight,
            "l2-norm-single": channel_l2_weight,
        },
    },
...
activation_criteria_map = {
    "local": {
        ...
        "channelwise": {
            ...
            "feature-map-similarity": channel_similarity_feature_map,
        },
    },
    "global": {"elementwise": {"l1-norm": global_activation_l1}},
}
