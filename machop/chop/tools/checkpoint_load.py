...
def load_lightning_ckpt_to_unwrapped_model(checkpoint: str, model: torch.nn.Module):
    ...
    try:
        src_state_dict = torch.load(checkpoint, map_location=torch.device("cpu"))[
            "state_dict"
        ]
    except:
        src_state_dict = torch.load(checkpoint)
...
def load_unwrapped_ckpt(
    model_short_name, mask, is_quantize, checkpoint: str, model: torch.nn.Module
):
    """
    Load a PyTorch state dict or checkpoint containing state dict to a PyTorch model.
    """
    print("has_quantized? : ", end="")
    print(is_quantize)
    ...
    if mask != None:
        if "vgg" in model_short_name:
            ...
            state_dict["layer3.0.downsample.0.parametrizations.weight.0.mask"] = mask[
                12
            ]
            ...
            state_dict["layer4.0.downsample.0.parametrizations.weight.0.mask"] = mask[
                17
            ]
    ...
    if is_quantize == True:
        ...
        for key, value in state_dict.items():
            ...
            new_key = new_key.replace(
                ...
                "feature_layers.0.parametrizations.weight.original",
            )
            new_key = new_key.replace(
                "feature_layers.3.weight",
                "feature_layers.3.parametrizations.weight.original",
            )
            new_key = new_key.replace(
                "feature_layers.7.weight",
                "feature_layers.7.parametrizations.weight.original",
            )
            new_key = new_key.replace(
                "feature_layers.10.weight",
                "feature_layers.10.parametrizations.weight.original",
            )
            new_key = new_key.replace(
                "feature_layers.14.weight",
                "feature_layers.14.parametrizations.weight.original",
            )
            new_key = new_key.replace(
                "feature_layers.17.weight",
                "feature_layers.17.parametrizations.weight.original",
            )
...
def load_model(
    ...
    if load_type == "pt":
        model = load_unwrapped_ckpt(
            model_short_name, mask, is_quantize, checkpoint=load_name, model=model
        )
