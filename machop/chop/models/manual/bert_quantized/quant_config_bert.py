...
def by_type_parser(config: dict, num_hidden_layers: int) -> dict:
    ...
    for i in range(num_hidden_layers):
        ...
        p_config[layer_entry] = create_layer_config(linear_qc, matmul_qc, layer_qc)
...
def parse_bert_quantized_config(config: str | dict, num_hidden_layers: int) -> dict:
    ...
    assert "by" in config, "Must provide a by field, either type or name"
    by = config["by"]
    match by:
        case "type":
