...
it as a Pytorch model, copy and paste this snippet also to ``lab4.py``.
   ...
   [Note] MASE integrates seamlessly with native Pytorch models.
...
3. | You may have noticed, one problem with the channel multiplier is
     that it scales all layers uniformly, ideally, we would like to be
     able to construct networks like the following: \```python # define
     a new model class JSC_Three_Linear_Layers(nn.Module): def
     **init**\ (self): super(JSC_Three_Linear_Layers, self).__init__()
     self.seq_blocks = nn.Sequential( nn.BatchNorm1d(16), nn.ReLU(16),
     nn.Linear(16, 32), # output scaled by 2 nn.ReLU(32), # scaled by 2
     nn.Linear(32, 64), # input scaled by 2 but output scaled by 4
     nn.ReLU(64), # scaled by 4 nn.Linear(64, 5), # scaled by 4
     nn.ReLU(5),
   | )
