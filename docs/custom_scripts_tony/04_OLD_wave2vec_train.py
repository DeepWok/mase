...
# Run original encoder before MASE modifications
encoder_outputs = encoder(input_values)
hidden_states_original = encoder_outputs["last_hidden_state"]
print("🔹 Original Encoder hidden states shape:", hidden_states_original.shape)
print("🔹 Original Encoder first hidden state values:", hidden_states_original[0, :5])
...
def count_transformer_layer_analysis_pass(mg, pass_args={}):
    transformer_layers = 0
    
    for node in mg.fx_graph.nodes:
        if node.op == "call_module" and "encoder.layers" in str(node.target).lower():
            logger.info(f"Found transformer encoder layer: {node.target}")
            transformer_layers += 1
        else:
            logger.debug(f"Skipping node: {node.target}")
    
    return mg, {"transformer_layer_count": transformer_layers}


def remove_transformer_layer_transform_pass(mg, pass_args={}):
    for node in list(mg.fx_graph.nodes):  # Convert to list to safely modify graph
        if node.op == "call_module" and "encoder.layers" in str(node.target).lower():
            logger.info(f"Removing transformer encoder layer: {node.target}")
            
            # Replace all users of the transformer layer with its input
            parent_node = node.args[0]
            logger.debug(f"Parent node: {parent_node}")
            node.replace_all_uses_with(parent_node)

            # Erase the transformer layer node
            mg.fx_graph.erase_node(node)
        else:
            logger.debug(f"Skipping node: {node.target}")
...
mg, _ = remove_transformer_layer_transform_pass(mg)
mg, analysis_out = count_transformer_layer_analysis_pass(mg)
logger.info(f"Transformer layer count after removal: {analysis_out['transformer_layer_count']}")

model = mg.model
mg.draw("wave2vec_transformed_analysis_pass_graph.svg")

# Run MASE-modified model after removing feature projection
encoder_outputs_mase = model(input_values)
hidden_states_mase = encoder_outputs_mase["last_hidden_state"]
print("🔹 MASE Model hidden states shape:", hidden_states_mase.shape)
print("🔹 MASE Model first hidden state values:", hidden_states_mase[0, :5])
...
def full_inference_beam(model, ctc_head, processor, input_values, beam_width=10):
    ...
    with torch.no_grad():
        ...
        encoder_outputs = model(input_values)
