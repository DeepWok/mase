...
from chop.passes.graph.analysis.quantization.cal_flexround import calibrate_flexround
...
def main():
    ...
    for n, p in mg_quant.model.named_parameters():
        if "weight" in n and "parametrizations" not in n:
            p.requires_grad = False  # Freeze original weights

    # Ensure only quantization params are trainable:
    trainable_params = [
        p for p in mg_quant.model.parameters() 
        if p.requires_grad and ("log_s1" in n or "log_S2" in n or "log_s3" in n)
    ]
