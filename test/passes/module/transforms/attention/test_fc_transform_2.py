...
# --------------------------------------------------
def train_model(
    model, train_dataset, eval_dataset, tokenizer, model_name="Model", num_epochs=1
):
...
def compare_models(do_train=True, num_epochs=1):
    ...
    print(
        f"Size Reduction: {original_size - transformed_size:.2f} MB ({100 * (original_size - transformed_size) / original_size:.2f}%)"
    )
    print("=" * 50)

    # Evaluation phase
    original_eval = evaluate_model(
        original_model, dataset["test"], tokenizer, "Original GPT-2"
    )
    transformed_eval = evaluate_model(
        transformed_model, dataset["test"], tokenizer, "FC-Transformed GPT-2"
    )
    ...
    results = {
        ...
        "evaluation": {"original": original_eval, "transformed": transformed_eval},
    }
    ...
    if do_train:
        ...
        transformed_train = train_model(
            transformed_model,
            dataset["train"],
            dataset["validation"],
            tokenizer,
            "FC-Transformed GPT-2",
            num_epochs,
        )
    ...
    print(
        f"Perplexity Impact: {((transformed_eval['perplexity'] - original_eval['perplexity']) / original_eval['perplexity'] * 100):.2f}%"
    )
    print()
    print(f"Original Model Eval Time: {original_eval['eval_time']:.2f} seconds")
    print(
        f"FC-Transformed Model Eval Time: {transformed_eval['eval_time']:.2f} seconds"
    )
    print(
        f"Speed Improvement: {((original_eval['eval_time'] - transformed_eval['eval_time']) / original_eval['eval_time'] * 100):.2f}%"
    )
    print()
    ...
    if do_train:
        print("\nTraining Results:")
        print(
            f"Original Model Training Time: {results['training']['original']['train_time']:.2f} seconds"
        )
        print(
            f"FC-Transformed Model Training Time: {results['training']['transformed']['train_time']:.2f} seconds"
        )
        print(
            f"Training Speed Improvement: {((results['training']['original']['train_time'] - results['training']['transformed']['train_time']) / results['training']['original']['train_time'] * 100):.2f}%"
        )
        print()
        print(
            f"Original Model Final Perplexity: {results['training']['original']['final_perplexity']:.4f}"
        )
        print(
            f"FC-Transformed Model Final Perplexity: {results['training']['transformed']['final_perplexity']:.4f}"
        )
        print("=" * 50)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Compare original GPT-2 with FC-transformed version"
    )
    parser.add_argument(
        "--train", action="store_true", help="Include training comparison"
    )
    parser.add_argument(
