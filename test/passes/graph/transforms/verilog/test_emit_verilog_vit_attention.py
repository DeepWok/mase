    ...
    for module in model.named_modules():
        if isinstance(module[1], Attention):
            ...
            new_module = ViTAttentionInteger(
                model_config["dim"], model_config["num_heads"], qkv_bias = model_config["query_has_bias"], q_config=q_config
            )
            logger.info(f"Replacing module: {module[0]}")
            dim = ori_module.head_dim* ori_module.num_heads

            qkv_weight = ori_module.qkv.weight.reshape(
                3, dim, dim)
            ...
            if has_bias:
                qkv_bias = ori_module.qkv.bias.reshape(
                    3, 1, dim)
...
def test_emit_verilog_vit_attention():
    ...
    model_args = {
        "vit_self_attention_integer": model_config
    }
    ...
    simulate(skip_build=False, skip_test=False, simulator="questa", trace_depth=5, waves=True)
